...
namespace DotNetNote.Models
{
    public class NoteRepository
    {
        ...
        public int SaveOrUpdate(Note n, BoardWriteFormType formType)
        {
            ...
            //[a]공통
            p.Add("@Name", value: n.Name, dbType: DbType.String); //sqlDbType을 지원하는 SqlDbType으로 변경
            ...
            switch (formType)
            {
                case BoardWriteFormType.Write:
                    ...
                    r = con.Execute("WriteNote", p, commandType: CommandType.StoredProcedure); //commandType: CommandText타입을 저장(쿼리, 저장프로시저, 테이블)
                    break;
        ...
        public int GetCountBySearch(string searchField, string searchQuery)
        {
            try
            {
                return con.Query<int>("SearchNoteCount", 
        ...
        public void UpdateDownCountById(int id)
        {
            ...
            con.Execute("Update Notes Set DownCount = DownCount+1 Where Id= @Id",p,commandType: CommandType.Text); //CommandType.Text: SQL텍스트 명령
        }
        ...
        /// 최근 올라온 사진 리스트 4개 출력
        /// </summary>
        public List<Note> DetNewPhotos()
        {
            string sql = "SELECT TOP 4 Id, Title, FileName, FileSize FROM Notes Where FileName Like '%.png' Or FileName Like '%.jpg' OR FileName Like '.%jpeg' OR FileName Like '%.gif' Order By Id Desc";
        ...
        {
            string sql = "SELECT TOP 3 Id, Title, Name, PostDate, FileName, FileSize, ReadCount, CommentCount, Step From Notes Where Category = @Category Order By Id Desc";
